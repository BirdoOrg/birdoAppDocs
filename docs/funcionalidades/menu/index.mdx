...
<details>
  ...
  1.1 `LineUp`: Referencia de lineup do SETUP;
  1.2 `ColorCard`: Cor de fundo do SETUP;
  1.3 `Referências`: Camada com referências do SETUP;
  - `Saída`: Define tipo de saída de mídia:
  1.1 `Imagens`: define como sequência de imagens (habilita a opção para escolher formato de imagem);
  1.2 `GIF`: define saída como GIF animado. (Habilita opção de frame rate);
  1.3 `Vídeo`: define saída como vídeo no formato `.mov`;
...
<details>
  ...
  1. `Duplo Clique`: Clique duas vezes em um item para enviar ele para a lista de selecionados;
  2. `Botão "►"`: Selecione um item desta lista e clique no botão "►" para jogar o item para lista de selecionados;
